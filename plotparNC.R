...
sim_id <- "NCPAR100000lin49x20T49.0TC"
...
sim_id <- "NCPAR100000lin49x50T49.0TC"
...
pdf('NCiterates_first_fig_par.pdf')
show(iterates_first_fig)
dev.off()

pdf('NCiterates_thin_fig_par.pdf')
show(iterates_thin_fig)
dev.off()

for (i in 1:NP)
{
  pdf(paste('NCden', paramsfn[i], 'fig_par.pdf',sep=''))
  show(den_fig[[i]])
  dev.off()
}
